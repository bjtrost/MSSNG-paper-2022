...
#####################################################################################################
# Generate the Illumina portion of the SV table to import into BigQuery for use in the MSSNG portal #
#####################################################################################################
...
metadata = pandas.read_csv(args.MSSNG_metadata_filename, sep="\t", index_col="Sample ID", low_memory=False, dtype=str)
...
for SV in SVs:
    ...
    if (SV["comment"] == "-" and int(SV["length"]) > args.max_common_size) or (SV["comment"] == "high quality rare" and int(SV["length"]) > args.max_rare_size): # Discard SVs that are not HQR and for which the size is >= 1 Mb
        continue

    Manta_any = ("MANTA" in SV["method"])
    DELLY_any = ("DELLY" in SV["method"])
    Manta_PASS = "MANTA" in SV["method"] and "PASS" in SV["MANTA_filter"]
    DELLY_PASS = "DELLY" in SV["method"] and "PASS" in SV["DELLY_filter"]

    small_DEL_filter = ((SV["sv_type"] == "DEL") and (int(SV["length"]) < 1000) & (Manta_any)) # Was originally: ((SVs["sv_type"] == "DEL") & (SVs["length"] < 1000) & (Manta_any | DELLY_PASS))
    large_DEL_filter = ((SV["sv_type"] == "DEL") and (int(SV["length"]) >= 1000) & Manta_PASS)
    DUP_filter = ((SV["sv_type"] == "DUP") & Manta_PASS)
    INS_filter = ((SV["sv_type"] == "INS") & Manta_any)
    INV_filter = ((SV["sv_type"] == "INV") & Manta_PASS & DELLY_any)
    complex_filter = ((SV["sv_type"] != "DEL") & (SV["sv_type"] != "DUP") & (SV["sv_type"] != "INS") & (SV["sv_type"] != "INV"))

    if small_DEL_filter or large_DEL_filter or DUP_filter or INS_filter or INV_filter or complex_filter:
        overlap = "{:.2f}|{:.2f}".format(float(SV["MANTA_ovlp"])/100, float(SV["DELLY_ovlp"])/100)
        EHdn_TR_size = EHdn_repeat_unit = EHdn_expansion = typeseq_priority = ""

        if SV["sv_type"] == "INS":
            start = str(int(SV["start"]) + 50)
            end = start # To compensate for bug in script that generates TSV files, which sets end = start + 1 (should be end = start)
        else:
            start = SV["start"]
            end = SV["end"]
