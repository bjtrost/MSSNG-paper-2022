...
dt.denom <- read.delim("ssc.transmission.count.singletons.2021-08-13.tsv", stringsAsFactors = F)
...
# ggsave("ssc.singletons.transmission.png", height = 6, width = 6)

dt.denom <- dt.denom[dt.denom$proband.transmitted < 40000, ]
manifest <- manifest[manifest$Sample.ID %in% dt.denom$Sample.ID, ]

dt.feat <- read.delim("ssc.transmission_features_singletons_2021-08-16.tsv", stringsAsFactors = F)
...
write.table(proband, "results/transmission.test.SSCProband.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
write.table(sibs, "results/transmission.test.SSCUnaffected.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggsave(sprintf("ssc.singletons.result.%s.png", Sys.Date()), width = 14, height = 8)
...
write.table(dt.feat, "SSC.singletons.features.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
write.table(dt.out, "results/burden.test.SSCProband.SSCUnaffected.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggsave("Proband.vs.Sibs.SSC.singletons.png", height = 6, width = 12)
