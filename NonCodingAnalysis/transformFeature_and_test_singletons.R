...
dt.denom <- read.delim("transmission.count.data.singletons.2021-08-13.tsv", stringsAsFactors = F)
...
# 
# ggsave("mssng.singletons.transmission.png", height = 6, width = 6)
...
write.table(dt.denom, "for_terra_transmission.count.data.singletons.2022-01-27.csv",
            sep=",", row.names=F, quote=F, col.names=T)

manifest <- manifest[manifest$Sample.ID %in% dt.denom$Sample.ID, ]
dt.feat <- read.delim("transmission_features_singletons_2021-08-16.tsv", stringsAsFactors = F)
...
write.table(dt.feat, "for_terra_transmission.features_singletons.2022-01-27.csv", sep=",", row.names=F, quote=F, col.names=T)
...
write.table(dt.out, "results/transmission.test.MSSNG.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
dt.out$BHFDR <- round(p.adjust(dt.out$p, method = "BH"), digits = 2)
dt.out$BHFDR[is.na(dt.out$BHFDR)] <- 1
write.table(dt.out, "results/transmission.test.MSSNG.MPX.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
dt.out$BHFDR <- round(p.adjust(dt.out$p, method = "BH"), digits = 2)
dt.out$BHFDR[is.na(dt.out$BHFDR)] <- 1
write.table(dt.out, "results/transmission.test.MSSNG.SPX.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggsave("FET.mssng.singletons.png", height = 8, width =14)
...
dt.out$BHFDR[is.na(dt.out$BHFDR)] <- 1
write.table(dt.out, "results/burden.test.MSSNG.MPX.SPX.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggplot(dt.out, aes(x = feat, y = or, color = BHFDR <= 0.25)) +
  ...
  scale_color_manual(values = c("#333333", "red")) + geom_hline(lty=2, yintercept = 1) +
  coord_cartesian(ylim=c(0.75, 1.35))
ggsave("SPXvsMPX.mssng.singletons.png", height = 6, width = 12)

write.table(dt.feat, "MSSNG.singletons.features.tsv", sep="\t", row.names=F, quote=F, col.names=T)

##### MSSNG vs SSC
dt <- rbind(read.delim("MSSNG.singletons.features.tsv", stringsAsFactors = F),
            read.delim("SSC.singletons.features.tsv", stringsAsFactors = F))
...
dt.out$BHFDR <- p.adjust(dt.out$p, method = "BH")
dt.out$BHFDR[is.na(dt.out$BHFDR)] <- 1
write.table(dt.out, "results/burden.test.MSSNGMPX.SSCProband.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)


p1 <- ggplot(dt.out, aes(x = feat, y = or, color = BHFDR <= 0.25)) +
  geom_errorbar(aes(ymin = low, ymax = up), width = 0, size = 3, alpha = .3) +
  geom_point() + theme_bw() + xlab("") + ylab("Odds ratio") +
  theme(axis.text.x = element_blank(), legend.position = "top") + ggtitle("SSC MPX(0) vs MSSNG MPX(1)") +
  scale_color_manual(values = c("#333333", "red")) + geom_hline(lty=2, yintercept = 1) +
  coord_cartesian(ylim = c(0.75, 1.35))
...
dt.out$BHFDR <- p.adjust(dt.out$p, method = "BH")
dt.out$BHFDR[is.na(dt.out$BHFDR)] <- 1
write.table(dt.out, "results/burden.test.MSSNGSPX.SSCProband.Singletons.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggsave("SSC.vs.MSSNG.singletons.png", height = 8, width = 14)
